...
class EventsDataByColumnResource(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            filters = [getattr(CurrentEvents, col) == val for col, val in zip(column_names, column_values)]
            data = session.query(CurrentEvents).filter(*filters).all()
