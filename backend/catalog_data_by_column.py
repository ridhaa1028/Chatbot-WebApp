...
from .models import Catalog # Import the Catalog, Catalog model from models.py

parser = reqparse.RequestParser()
parser.add_argument('column_name', type=str, action='append')
parser.add_argument('column_value', type=str, action='append')

class CourseDataByColumnResource(Resource):
    ...
    def get(self):
        ...
        valid_columns = ['subj', 'crse', 'title', 'prereq', 'description', 'level']
        ...
        try:
            ...
            filters = [getattr(Catalog, col) == val for col, val in zip(column_names, column_values)]
            data = session.query(Catalog).filter(*filters).all()
